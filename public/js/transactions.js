...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('addTransactionBtn').addEventListener('click', function(event){
        event.preventDefault();
        const amountElement = document.getElementById('transactionAmount');
        const addTransactionErrorElement = document.getElementById('addTransactionError');
        ...
        .then(conversion_rate => {
            ...
            if (isNaN(conversionRate)) {
                addTransactionErrorElement.innerText = 'Error getting conversion rate from the API, please try again later.';
                addTransactionErrorElement.style.display = 'block';
                return;
            } else {
                addTransactionErrorElement.style.display = 'none';
            ...
            const totalTransactionAmount = (conversionRate * amountValue).toFixed(2);
        ...
        .then(data => {
            if (data.success){
                ...
                `
                transactionsList.insertBefore(transactionRow, transactionsList.firstChild);
            }
            else{
                addTransactionErrorElement.innerText = 'Transaction addition failed.';
                addTransactionErrorElement.style.display = 'block';
            }
        })
        .catch(error => {
    ...
    document.getElementById('transactionsList').addEventListener('click', async function(event){
        if(event.target.classList.contains('delete-btn')){
            const transactionId = event.target.getAttribute('data-id');

            try{
                const response = await axios.post(`/transactions/delete/${transactionId}`);
